...
def detect_class_met(entrada: str):
    ...
    # Detecta métodos con tipo de retorno, excluyendo líneas que empiecen por "new"
    patron_metodo = re.compile(
    r'^(\s*)'                                     # (1) Espacios iniciales
    r'(?!new\b)'                                  # (1b) NO permitir que empiece por "new"
    r'(?!else\s+if\s*\()'                         # (1c) Negación de "else if"
